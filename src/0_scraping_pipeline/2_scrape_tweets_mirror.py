...
def pick_days(start_year, start_month, end_year, end_month):
    days = []
    y, m = start_year, start_month
    while (y < end_year) or (y == end_year and m <= end_month):
        _, last = calendar.monthrange(y, m)
        choices = random.sample(range(1, last + 1), 2)
        for d in sorted(choices):
...
SCRAPE_DATES = pick_days(2024, 9, 2025, 3)
...
async def extract_tweet(tweet):
    for attempt in range(MAX_RETRIES):
        try:
            ...
            username    = await tweet.locator(".fullname-and-username a.username")\
                                       .nth(0).inner_text()
            ...
            retweeted = await tweet.locator(".retweet-header").inner_text() if retu else None
            ...
            stats = await tweet.locator(".tweet-stats span.tweet-stat").evaluate_all(
            ...
            if await tweet.locator(".tweet-date a").count():
                timestamp = await tweet.locator(".tweet-date a")\
            ...
            if await tweet.locator(".quote-text").count():
                quote_text = await tweet.locator(".quote-text")\
                                        .nth(0).inner_text()
            if await tweet.locator(".quote .fullname-and-username a.username").count():
    ...
    try:
        while True:
            try:
                ...
                log.error(f"Page.goto timeout on {url}; retrying later")
                fails += 1
            ...
            # CORRECTED: await get_attribute() to retrieve a string
    ...
    async with async_playwright() as p:
        ...
        for acct in ACCOUNTS:
            for day in SCRAPE_DATES:
                ...
                url      = state["cursor"] or build_search_url(NITTER_BASE, acct, day, day + timedelta(days=1))
                csv_file = os.path.join(RAW_DIR, acct, f"{ds}_{acct}.csv")

                try:
                    await scrape_nitter_mentions(ctx, acct, url, NITTER_BASE, csv_file)
